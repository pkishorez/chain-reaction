        ...
        on: {
          PLAYERS_SELECTED: {
            ...
            actions: assign((ctx, event) => {
              ...
              ctx.grid = { rows, cols };
              ctx.game = range(rows * cols).reduce((acc, posId) => {
                acc[posId] = {
                  dots: [],
              ...
              ANIMATION_DONE: [
                {
                  ...
                  actions: ["setNextPlayer", "checkIfGameOver"],
                },
              ],
            },
  ...
  },
  {
    ...
    actions: {
      resetGame: assign((context) => {
        context.blastQueue = defaultValue.blastQueue;
        context.currentPlayerIndex = defaultValue.currentPlayerIndex;
        context.game = defaultValue.game;
        context.gameOver = defaultValue.gameOver;
        context.grid = defaultValue.grid;
        context.players = defaultValue.players;
      }),
      ...
      setupAnimationQueue: assign((ctx, event) => {
        ...
        const newBlock = {
          ...
          dots: [...(currentBlock?.dots ?? []), { id: newDotId, isNew: true }],
        };
      ...
      blastAnimationTicker: assign((ctx) => {
        ...
        for (let entry of ctx.blastQueue) {
          ...
          if (blockUnderTest.dots.length >= getBurstCount(ctx.grid, posId)) {
      ...
      setNextPlayer: assign((ctx) => {
        ...
        for (let i = 1; i < ctx.players.length; i++) {
          let nextPlayer = (ctx.currentPlayerIndex + i) % ctx.players.length;
      ...
      checkIfGameOver: actions.pure((ctx) => {
        ...
        const currentValidPlayers = new Set(
          Object.values(ctx.game)
            ...
            .filter(
              (playerIndex): playerIndex is number => playerIndex !== undefined
            )
        );
